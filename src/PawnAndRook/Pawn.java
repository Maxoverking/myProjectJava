...
public class Pawn {
    ...
    // сеттеров отдельно для строки и столбца не будет - можно только менять
    // координаты вместе
    ...
    public void setCoordinates(int row, int column) {
        checkCoordinates(row, column);
        int rowFront = row - this.row;
        int colLeftRight = column - this.column;
        if (rowFront <= 0) {
            throw new IllegalArgumentException("Ходить назад нельзя");
        } else if (rowFront < 3) {
            System.out.println("Ходить вперед можно на " + rowFront + "ве клетки");
        } else {
            throw new IllegalArgumentException("Ходить нельзя");
        }
    ...
    private static void checkCoordinates(int row, int column) {
        if (row < 1 || row > 8) {
            ...
            // выбрасываем её самостоятельно - теперь наш собственный код может сообщать об
            // ошибке
